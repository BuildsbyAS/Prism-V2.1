...
drop policy if exists "editors read responses" on public.responses;
...
drop policy if exists "editors read answers" on public.response_answers;
...
create table if not exists public.notifications (
  ...
  event_type        text not null check (
    event_type in (
      'form_published', 'vote_received', 'collaborator_added',
      'collaborator_removed', 'form_expired', 'final_results'
    )
  ),
...
begin
  ...
  loop
    insert into public.notifications (
      recipient_email, form_id, event_type, event_key,
      title, message, action_path, created_at
    )
    values (
      v_email, new.form_id, 'vote_received', 'vote:' || new.id::text,
      'New vote received',
...
begin
  if auth.uid() is null or not public.is_noon_user() then return; end if;

  insert into public.notifications (
    recipient_email, form_id, event_type, event_key,
    title, message, action_path, created_at
  )
  select v_email, f.id, 'form_expired',
  ...
  select v_email, f.id, 'final_results',
...
returns table (
  id uuid, form_id uuid, event_type text, title text, message text,
  action_path text, created_at timestamptz, read_at timestamptz
)
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  return query
  select n.id, n.form_id, n.event_type, n.title, n.message,
         n.action_path, n.created_at, n.read_at
